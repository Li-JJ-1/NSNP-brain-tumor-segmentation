...
def val_loop(model, criterion, val_loader, device):
    ...
    dice3 = dice3_val / len(val_loader)# 这里算的就是平均值 每得到一个就除以长度
    return {'loss': loss, 'dice1': dice1, 'dice2': dice2, 'dice3': dice3}
...
def main(args):
    ...
    #     ToTensor()
    # ]))
    train_loader = DataLoader(dataset=train_dataset, batch_size=args.batch_size, num_workers=10,   # num_worker=4
                              shuffle=True, pin_memory=True)
    val_loader = DataLoader(dataset=val_dataset, batch_size=args.batch_size, num_workers=10, shuffle=False,#尽量和cpu一样，慢慢的递增
                            pin_memory=True)
    # test_loader = DataLoader(datasets=test_dataset, batch_size=args.batch_size, num_workers=12, shuffle=False,
    #                          pin_memory=True)
    ...
    print(torchsummary.summary(model=model, input_size=((4, 128, 128, 64))))

    criterion = Loss(n_classes=4, weight=torch.tensor([0.25, 0.25, 0.25, 0.25])).to(device)
    optimizer = optim.Adam(model.parameters(), lr=args.lr, weight_decay=1e-5) #1e-5  1e-8  5e-4
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=1e-2)#1e-3  最好为2e-4
    parser.add_argument('--min_lr', type=float, default=0.0002)#0.002
    parser.add_argument('--data_path', type=str, default=r'datasets/BraTS2021/BraTS2021_dataset/')
    parser.add_argument('--train_txt', type=str, default=r'datasets/BraTS2021/train.txt')
    parser.add_argument('--valid_txt', type=str, default=r'datasets/brats2021/valid.txt')
    parser.add_argument('--train_log', type=str, default=f'results/{MODEL_NAME}/{MODEL_NAME}.train_log_2.txt')
    parser.add_argument('--weights', type=str, default=f'results/{MODEL_NAME}/{MODEL_NAME}_2.pth')
