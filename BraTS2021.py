...
class RandomCrop(object):
    ...
    Args:
    output_size (int): Desired output size 希望的输出图像大小
    """
    ...
    def __call__(self, sample):
        ...
        # label = label[w1:w1 + self.output_size[0], h1:h1 + self.output_size[1], START:END]#从标签中裁剪出一个子区域
        # image = image[:, w1:w1 + self.output_size[0], h1:h1 + self.output_size[1], START:END]#从图像中裁剪出一个子图像  第一个冒号表示选择所有的通道
...
class RandomRotFlip(object):
    """
    Crop randomly flip the datasets in a sample
    Args:
...
#
#         label = sitk.GetArrayFromImage(sitk.ReadImage(str(self.paths[item]) + '/' + str(self.paths[item]).split('/')[-15:] + '_seg.nii.gz')).transpose(1, 2, 0)
#         # print(label.shape)
#         # 堆叠四种模态的图像，4 x (H,W,D) -> (4,H,W,D)
#         images = np.stack(
#             [sitk.GetArrayFromImage(sitk.ReadImage(str(self.paths[item]) + '/' + str(self.paths[item]).split('/')[-15:] + modal + '.nii.gz')).transpose(1, 2, 0) for modal in
#              modalities],
#             0)  # [240,240,155]
#         # 数据类型转换
...
if __name__ == '__main__':
    ...
    test_txt = "./datasets/BraTS2021/test.txt"
    size = (240, 240, 64)
