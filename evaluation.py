...
from networks.MyNet.baselineMDSP import BaselineRSA
from networks.MyNet.baselineCSA import BaselineCSA
...
def val_loop(model, criterion, loader, device):
    ...
    with torch.no_grad():
        ...
        for images, masks in pbar:
            ...
            with open(f"results/test/{MODEL_NAME}.txt", 'a+') as f:
    ...
    ds_tc = ds_tc / len(loader)#Hausdorff
...
def main(args):
    ...
    # train_loader = DataLoader(datasets=train_dataset, batch_size=args.batch_size, num_workers=8,  # num_worker=4
    #                           shuffle=True, pin_memory=True)
    # val_loader = DataLoader(dataset=val_dataset, batch_size=args.batch_size, num_workers=10, shuffle=False,
    #                         pin_memory=True)
    test_loader = DataLoader(dataset=test_dataset, batch_size=args.batch_size, num_workers=10, shuffle=False,
                             pin_memory=True)
    ...
    elif MODEL_NAME == 'baselineCSA':
        model = BaselineCSA(in_channels=4, num_classes=4).to(device)
...
if __name__ == '__main__':
    ...
    # MODEL_NAME = 'baselineCSA'
    parser = argparse.ArgumentParser()
    parser.add_argument('--num_classes', type=int, default=4)
    parser.add_argument('--seed', type=int, default=21)
    parser.add_argument('--epochs', type=int, default=10)
    parser.add_argument('--warmup_epochs', type=int, default=5)
    parser.add_argument('--batch_size', type=int, default=1)#测试集只需要1
    parser.add_argument('--lr', type=float, default=2e-4)#1e-3
    parser.add_argument('--min_lr', type=float, default=0.0002)#0.002
    parser.add_argument('--data_path', type=str, default='./datasets/BraTS2021/BraTS2021_dataset')
    # parser.add_argument('--val_data_path', type=str, default='../datasets/brats2020/val_data/data')
    parser.add_argument('--train_txt', type=str, default='./datasets/BraTS2021/train.txt')
    parser.add_argument('--valid_txt', type=str, default='./datasets/BraTS2021/valid.txt')
    parser.add_argument('--test_txt', type=str, default='./datasets/BraTS2021/test.txt')
    parser.add_argument('--train_log', type=str, default=f'results/{MODEL_NAME}/{MODEL_NAME}.train_log.txt')
    parser.add_argument('--weights', type=str, default=f'results/{MODEL_NAME}/{MODEL_NAME}.pth')#权重的路径要和train.py的一样
